def send_email(message, recipient, *, sender="university.help@gmail.com"):
    ...
    if sender == recipient:
        ...
        elif not is_valid_email(sender) and not is_valid_email(recipient):
            print(f"Невозможно отправить письмо с адреса {sender} на адрес {recipient}. Оба адреса некорректны.")
        elif not is_valid_email(sender):
            print(f"Невозможно отправить письмо. Некорректный адрес отправителя: {sender}.")
        elif not is_valid_email(recipient):
            print(f"Невозможно отправить письмо. Некорректный адрес получателя: {recipient}.")
