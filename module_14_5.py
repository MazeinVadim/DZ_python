...
# Дополнение кнопки "Регистрация" в главное меню
keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
buttons = ["Рассчитать", "Информация", "Регистрация"]
...
async def start_message(message):
    await message.reply('Привет! Я бот, помогающий твоему здоровью. Также Я продаю 3D модели. '
...
async def get_buying_list(message):
    ...
    for product in products:
        await message.reply_photo(product[4],f"Название: {product[1]} | Описание: {product[2]} | Цена: {product[3]}")
...
async def set_age(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply("Регистрация завершена!", reply_markup=keyboard)
        await state.finish()
    except ValueError:
...
if __name__ == "__main__":
    initiate_db()
    # Пополнение таблицы продуктами
    def populate_products():
        conn = sqlite3.connect('products.db')
        cursor = conn.cursor()
        products = [
            ('Продукт 1', 'Описание продукта 1', 100, 'https://t.me/blender_spb/73?single'),
            ('Продукт 2', 'Описание продукта 2', 200, 'https://t.me/blender_spb/79'),
            ('Продукт 3', 'Описание продукта 3', 300, 'https://t.me/blender_spb/85?single'),
            ('Продукт 4', 'Описание продукта 4', 400, 'https://t.me/blender_spb/113?single')
        ]
